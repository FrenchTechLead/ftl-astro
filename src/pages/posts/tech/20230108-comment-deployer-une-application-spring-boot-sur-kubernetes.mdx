...
publishDate: January 08, 2023
authorName: "@FrenchTechLead"
authorSocial: "https://twitter.com/FrenchTechLead"
postImageUrl: https://frenchtechlead.com/assets/blog/tech/20201023-git-pour-debutant/0.png
postImageAlt: Comment déployer une application spring-boot et Postgres sur Kubernetes
postImageWidth: 700
postImageHeight: 350
keywords:
 - Kubernetes
...
<Image src={img0} t='Comment déployer une application spring-boot et Postgres sur Kubernetes' solo={true}/>
...
> Docker desktop vient avec un cluster kubernetes intégré que vous pouvez activer depuis les paramètres, vous n'aurez pas besoin d'installer kubernetes et kubectl si vous choisissez d'utiliser Docker Desktop.
<Separator/>

## Configuration réseau
### Ouverture de votre serveur local sur internet (Free)
Pour que votre serveur local soit accessible depuis internet, vous devez autoriser le trafic entrant dans votre box internet et le redériger vers votre serveur, nous allons décrire comment faire cela avec l'opérateur free.    
...
> Une fois l'adresse IP attribuée, vous devez ouvrir les ports 80 et 443 de votre box et redériger le traffic vers votre serveur.
...
Une fois connecté dirigez-vous sur **Paramètre de la Freebox** puis **Gestion des ports** puis ajoutez les deux règles suivantes en adaptant la première ligne à votre situation. (adresse de votre serveur)
...
3. Appliquer les deux scripts avec kubectl
```sh
...
3. Appliquer les trois scripts avec kubectl
```sh
...
L'émetteur de production de Let's Encrypt a des limites de débit très strictes. Lorsque vous expérimentez et apprenez, il est très facile de atteindre ces limites. En raison de ce risque, nous commencerons avec l'émetteur de staging de Let's Encrypt, et une fois que nous serons sûrs qu'il fonctionne, nous passerons à l'émetteur de production.
...
2. Créez un fichier **9-ingress-ressource.yaml** avec le contenu suivant:
```yaml
...
Cependant le certificat ayant été émis par l'autorité de certification staging de let's encrypt, votre navigateur web devrait vous avertir du rique d esécurité encouru.  
Pour switcher sur un certificat de production, vous devez editer le script **9-ingress-ressource.yaml** en modifiant  **cert-manager.io/issuer: "bien-parler-letsencrypt-staging-issuer"** par **cert-manager.io/issuer: "bien-parler-letsencrypt-prod-issuer"**, ensuite supprimer la secret **bien-parler-tls** afin qu'une nouvelle demande de certificat soit émise par le cert-manager:
```sh
